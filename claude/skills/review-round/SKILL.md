---
name: review-round
description: "`/review-round <N> [/code-review 인자...]`. implement PR 에 Claude·Codex 이중 리뷰 라운드 한 번을 돌리고 결과를 PR 코멘트 하나로 남긴다. 유료 엔진을 띄우므로 사람이 부른다."
...
- **`ultra` 는 실행을 멈춘다.** 사용자에게 `/code-review ultra` 를 직접 치라고 말하고, 그 결과가 오면 [4] 부터 이어 간다.
...
- [ ] 2  상태 잡기, 라운드 번호 정하기
- [ ] 3  ROUND_BASE 에 Claude·Codex 를 띄우고 공동 모니터를 걸고 턴을 끝냄
- [ ] 4  (알림 뒤) 두 finding 목록 추출·점검·중복 제거
...
**3b. Claude 와 Codex 를 백그라운드로 띄운다. 인라인 금지.** `/code-review` 는 `disable-model-invocation` 이라 `Skill` 도구로는 못 부르지만, 그 플래그는 *모델이 자기를 부르는 것* 만 막는다. `claude -p` 는 사람이 친 것과 같은 슬래시 확장 경로를 타서 진짜로 리뷰한다 (evidence.md).
...
Codex 는 두 번째 백그라운드 프로세스로. `$review-agent` 가 읽기 전용 결함 우선 계약을 주고, `--output-schema` 가 최종 응답을 산문 긁기 없이 바로 쓸 수 있게 한다.
...
- `<args>` 는 「인자」 에서 effort 를 기본값 처리한 문자열이다. 호출자의 원문 꼬리가 아니다.
- **`run_in_background: true` 는 필수다.** 진짜 리뷰는 포그라운드 Bash 한도를 넘길 수 있고, 완료 알림이 [4] 를 깨운다.
- `.done` 하나가 프로세스 exit code 하나이고 모니터의 정지 신호다. 이전 산출물을 먼저 전부 지운다. 지난 라운드의 잔재가 이번 결과로 위장할 수 있다.
...
모니터는 두 리뷰어가 `.done` 을 쓰면 끝난다. 그 다음 **턴을 끝내고 알림을 기다린다.** 폴링하지 않고, sleep 루프를 돌지 않고, 두 번째 리뷰를 띄우지 않는다.
...
- `AskUserQuestion` 을 쓰지 않는다. 코멘트에 없는 분석·추가 선택지·추천을 덧붙이지 않는다. 결정은 다음 `/implement <N>` 지시로 돌아온다.
...
이 Claude 진입점은 백그라운드 Bash 와 Monitor 알림으로 바깥 상태 기계를 소유한다. Codex 쪽 형제 `~/.codex/skills/review-round/SKILL.md` 가 사람이 Codex 에서 `$review-round` 를 부를 때 같은 라운드를 소유한다. 두 진입점은 `~/.agents/skills/agent-loop/review-round/` 의 결정적 헬퍼와 코멘트 계약을 공유한다. 어느 런타임에서든 nested Codex 프로세스는 `$review-agent` 를 읽기 전용으로 부르고, 브랜치를 쓰는 것은 바깥 진입점만이다.
