...
- [조건별 기준](routes.md)의 전후 비교 대상이면 PR base를 fetch하고 정확한 ref/SHA로 `scripts/base-worktree.sh <ref>`를 실행한다. head의 메인 체크아웃을 stash·switch하지 않는다. base에서도 필요한 앱과 API만 시작한다.
