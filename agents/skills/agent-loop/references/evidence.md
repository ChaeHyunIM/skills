...
사용자는 반복적인 PR 리뷰에서 하위 이슈가 아직 구현하지 않기로 한 범위까지 버그로 지적되는 문제를 관찰했다. 개별 구현의 실제 동작과 전문 스킬에 따른 코드 품질 검증은 `implement`·`verify`가 맡는다. 사용자 흐름 전체의 리뷰는 사용자가 시점과 범위를 판단해 Claude의 `/code-review`를 직접 호출한다. 별도의 리뷰 실행·라운드 관리 절차는 두지 않는다.
