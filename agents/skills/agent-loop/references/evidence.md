...
**검증 서버의 포트가 고정돼 있어 두 worktree에서 동시에 검증할 수 없었다.**

2026-09-18에 확인했다. 두 번째 worktree에서 `dev-servers.sh start`를 실행하면 `port … is taken` 오류가 났다. web·admin·API·Metro 모두 같았다. doko는 API의 CORS와 better-auth `trustedOrigins`가 `localhost:3000`·`3001`만 허용했다. 포트를 바꾸면 로그인이 거부돼 스크립트만 고쳐서는 해결되지 않았다.

당시에는 체크아웃마다 슬롯을 하나씩 할당하고, 포트를 `PORT_<app> + PORT_STRIDE × 슬롯`으로 계산했다. 별도로 clone한 저장소끼리는 서로의 worktree 목록을 알 수 없어 같은 머신의 모든 체크아웃이 슬롯 할당 정보를 공유했다. 0번 슬롯은 사람이 직접 띄우는 개발 서버용으로 비워 뒀다. 임시 저장소 두 개에서 동시에 시작했을 때 서로 다른 슬롯과 포트를 받는 것도 확인했다.

포트를 바꿀 수 없는 저장소에서는 `PORT_STRIDE=0`으로 설정해 한 번에 한 체크아웃만 실행하도록 했다. 슬롯 상한(20)에 도달하거나 슬롯마다 시뮬레이터를 띄워야 할 때는 이 방식을 다시 검토하기로 했다. 이후 아래 변경에서 슬롯 관리 스크립트를 제거했다.

**앱 실행까지 전역 스크립트로 처리하면서 스크립트와 설정, 설명 문서가 계속 늘었다.**

2026-09-18 당시 `verify`에는 검증 결과의 판정 기준, PR 기록 방법, 앱 실행 방법이 섞여 있었다. 프로젝트마다 다른 실행 방법을 지원하려다 보니 `dev-servers.sh`(179줄)·`slots.sh`·`base-worktree.sh`·`e2e.sh`·`check-env.sh`가 필요해졌고, config에는 `START_<app>`·`PORT_STRIDE` 같은 설정이 추가됐다. `config.example`은 사실상 doko 설정이었고, `errors.md` 21줄 중 약 8줄은 이 스크립트의 오류를 설명했다. 전역 스킬이 프로젝트 검증 스킬에 실행을 맡기면서, 프로젝트 검증 스킬은 다시 전역 스크립트를 호출하는 구조였다.

전역에는 Git·GitHub 작업용 스크립트(`prepare-worktree.sh`·`check-worktree.sh`·`load-config.sh`·`verified-head.sh`)와 공통 규칙, 검증 결과의 판정 기준, PR 기록 형식만 남겼다. 앱 실행·조작 방법과 완료 조건별로 남길 기록물은 저장소의 프로젝트 검증 스킬에 적도록 했다. 빈 포트 선택, 프로세스 추적, 검증 섹션 작성, fast-forward는 모델이 직접 처리한다. 위 포트 슬롯 규칙과 `evidence-block.mjs`·`sync-worktree.sh`도 이때 제거했다.

이 스크립트들을 제거한 뒤 실제 티켓을 구현해 본 기록은 아직 없다. 모델 성능이 높아져 스크립트 없이도 처리할 수 있다고 판단한 것이며, 실제 작업에서는 더 확인해야 한다.

같은 문제가 재발하면 해당 스크립트만 Git 이력에서 복구한다. 두 worktree의 서버가 같은 포트를 쓰거나, 다른 작업의 프로세스를 종료하거나, 검증 섹션의 형식이 잘못돼 `verified-head.sh`가 `missing`을 반환하거나, 동기화 중 로컬 커밋을 잃는 경우가 해당한다. 성능이 낮은 모델로 이 스킬을 실행할 때도 스크립트가 필요한지 다시 검토한다.
...
**같은 내용의 섹션이 PR마다 다른 이름으로 쓰였고, 본문 대부분이 코드를 다시 설명했다.**

- 확인한 사실(2026-09-18): doko-monorepo PR 200개(2026-07-27 ~ 09-17)를 읽었다. 변경 내용 섹션은 `변경`·`구현`·`무엇이 달라지나`·`한 일` 등 20가지가 넘는 이름으로 쓰였고, 헤딩이 없는 본문도 34개였다. 마이그레이션은 61개, 배포 순서는 26개 PR에서 언급됐지만 위치가 정해져 있지 않았다. 본문 길이 중앙값은 약 1,700자였다. PR #383의 구현 설명 한 문단은 `packages/core/src/modules/gift-shop/repository.ts`의 주석과 같은 내용이었다.
- 현재 규칙: PR 본문은 사람이 읽는다. 에이전트는 코드베이스에서 맥락을 얻으므로 코드로 알 수 있는 내용은 본문에 쓰지 않는다. 섹션 이름을 고정하고, 배포 영향은 "없음"이라도 반드시 채운다.
- 다시 확인할 상황: 리뷰어가 본문만으로 판단하지 못해 diff 설명을 따로 요청하는 일이 반복되면 `리뷰 포인트`의 기준을 다시 본다.
