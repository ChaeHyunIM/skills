---
name: land
description: Drains the merge queue — every issue labelled agent-merge-ready — by merging its PRs in queue order, resolving conflicts as an assembly of both sides' documented intents, verifying each item with the app typecheck before it lands, and posting one Korean drain report. The label is the human's merge signature and $land is the loop's only sanctioned merge path. Use only when the user explicitly invokes `$land` (whole queue) or `$land ISSUE_NUMBERS` (a subset). Never merges an unlabelled PR and never resolves an intent collision.
---
...
- Resolve each issue's PR with `--search "Closes #<N>"`. If GitHub search indexing lags, fall back
  to matching `agent/issue-<N>-*` in the full PR list.
- Pin `REPO` and each issue's worktree. Recreate a missing worktree with
  `~/.codex/skills/implement/scripts/prepare-worktree.sh <N> <slug>` (no base argument — rework mode).
...
BASE=$(gh pr view <PR> --json baseRefName -q .baseRefName)   # fresh — never [2]'s value
```
...
After the last item lands, read `~/.codex/skills/tidy-merged/SKILL.md` and follow `$tidy-merged`.
Its script deletes a worktree only
