...
One name per thing. The three skills use only this vocabulary.
...
2. Copy `routeTree.gen.ts` from the main checkout — it is gitignored, so a new worktree lacks it.
   Never run `tsr generate`; it silently drops the `declare module` block. Both admin and doko.
...
- Typecheck through the app script only: `pnpm check-types:doko` · `:doko-app` · `:admin` · `:api`.
...
- Commit through the `$commit` skill only. Never run `git commit` directly. The one exception is
  **completing a conflict resolution** in `$land` (`git rebase --continue` · `git commit --no-edit`):
  a resolution authors nothing, so the authored-change pipeline does not apply.
