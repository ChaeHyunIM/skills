...
**Read `~/.codex/skills/agent-loop/CONTRACT.md` before starting** — the labels and output convention of
...
- **Design/UX truth = the Figma node** (when the surface has a separate design, e.g. doko-app). The node
...
1. **Collect the Figma node links** covering the work — from the conversation, or ask the user. For
   ...
   to bind to.
2. **Read the design as the PRD.** Walk the frames and flows with the available Figma connector or
   MCP's structure/metadata and screenshot capabilities:
...
**Read `~/.codex/skills/to-tickets/references/slicing-rules.md` and cut according to it.** It defines the
